...
const DropdownSearch = ({ setSearchQuery, searchValue, setSelectedCategory, setCurrentPage, setIsSelected, link }) => {
  ...
  recentSearch = useMemo(() => {
    let filterSearch = recentSearch?.reverse().filter(search => search?.toLowerCase() === inputValue?.toLowerCase());
    console.log(filterSearch)
    recentSearch = recentSearch?.filter(search => !filterSearch?.includes(search));
    recentSearch = [filterSearch[0]]?.concat(recentSearch).filter(el => el);
    return recentSearch
  }, [recentSearch])
